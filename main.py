...
def background_parse_data(db: Session):
    items = parse_data()
    for item in items:
        create_item(db, item["name"], item["description"], item["url"])
...
@app.post("/items")
async def create_new_item(name: str, description: str, url: str, db: Session = Depends(get_db)):
    return create_item(db, name, description, url)
